...
JsonViewer.prototype.renderChildren = function (theme, key, val, right, indent, left, dateTime) {
    let self = this;
...
JsonViewer.prototype.parse = function (dataObj, parent, indent, theme) {
    ...
    this.forEach(dataObj, function (val, key) {
        ...
        if (!isObject(val) && !isArray(val)) {
            ...
            self.renderChildren(theme, key, val, right, indent, left, false);
        }
    });
}
...
JsonViewer.prototype.render = function () {
    ...
    let key = 'object';
    parent.classList.add(theme + 'con')
    ...
    this.renderFirst(theme, key, data, right, indent, left, dateTime);
}




JsonViewer.prototype.renderFirst = function (theme, key, val, right, indent, left, dateTime) {
    let self = this;
    let folder = this.createElement('span');
    let rotate90 = this.options.expand ? 'rotate90' : '';
    let addHeight = this.options.expand ? 'add-height' : '';
    ...
    left.innerHTML = dateTime

    val.forEach((v, i) => {
        let newNode = new Node('span').addClass('no-pointer-event')
        if (isString(v)) {
            newNode.addText(v).addClass(theme + 'rightString')
        }
        if (isNumber(v)) {
            newNode.addText(v).addClass(theme + 'rightNumber')
        }
        if (isBoolean(v)) {
            newNode.addText(v).addClass(theme + 'rightBoolean')
        }
        if (isNull(v)) {
            newNode.addText('null').addClass(theme + 'rightNull')
        }
        if (isUndefined(v)) {
            newNode.addText('undefined')
        }
        if (isArray(v)) {
            newNode.addText('Array[' + v.length + ']')
        }
        if (isObject(v)) {
            newNode.addText('Object{' + Object.keys(v).length + '}')
        }
        newNode.appendTo(left)
        if (i < val.length - 1) {
            new Node('span').addClass('no-pointer-event').addText(', ').appendTo(left)
        }
    })
